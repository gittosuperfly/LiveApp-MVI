...
object Printer {
    ...
    fun printJsonRequest(
        ...
        if (builder.logger == null) ILog.log(
            builder.type,
            tag,
            REQUEST_UP_LINE,
        ...
        if (builder.level == Level.ALL || builder.level == Level.BODY) {
            logLines(
                builder.type,
                tag,
        ...
        if (builder.logger == null) ILog.log(builder.type, tag, END_LINE, builder.isLogHackEnable)
    }
    ...
    fun printJsonResponse(
        ...
        if (builder.logger == null) {
            ILog.log(builder.type, tag, RESPONSE_UP_LINE, builder.isLogHackEnable)
        }
        ...
        if (builder.logger == null) {
            ILog.log(builder.type, tag, END_LINE, builder.isLogHackEnable)
        }
    }
    ...
    private fun getRequest(level: Level, headers: Headers, method: String): Array<String> {
        val log: String
        val loggableHeader = level == Level.HEADERS || level == Level.ALL
    ...
    private fun logLines(
        ...
        for (line in lines) {
            ...
            for (i in 0..lineLength / maxLogSize) {
                ...
                if (logger == null) {
                    ILog.log(type, tag, DEFAULT_LINE + line.substring(start, end), useLogHack)
                } else {
    ...
    @Synchronized
    fun printFailed(tag: String, builder: LogInterceptor.Builder) {
        ILog.log(builder.type, tag, RESPONSE_UP_LINE, builder.isLogHackEnable)
        ILog.log(builder.type, tag, DEFAULT_LINE + "Response failed", builder.isLogHackEnable)
        ILog.log(builder.type, tag, END_LINE, builder.isLogHackEnable)
    }
