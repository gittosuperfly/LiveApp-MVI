...
public abstract class BaseDialog extends DialogFragment {
    ...
    private float dimAmount = 0.3f;//灰度深浅
    private int gravity = Gravity.CENTER;//显示的位置
    private boolean outCancel = true;//是否点击外部取消
    @StyleRes
    protected int theme = R.style.DialogStyle; // dialog主题
    @StyleRes
    ...
    private void initParams() {
        ...
        if (window != null) {
            ...
            switch (gravity) {
                ...
                case Gravity.CENTER: {
                    if (animStyle == 0) {
                        animStyle = R.style.CenterAnimation;
                    }
                }
                default:
                    break;
            ...
            //设置dialog宽度
            if (width == 0) {
                lp.width = ViewUtils.getDisplayWidth(requireActivity()) - 2 * ViewUtils.dip2px(margin);
            } else if (width == -1) {
                lp.width = WindowManager.LayoutParams.WRAP_CONTENT;
            } else {
                lp.width = ViewUtils.dip2px(width);
            }
            ...
            if (height == 0) {
                ...
                lp.height = ViewUtils.dip2px(height);
            }
